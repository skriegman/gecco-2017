...
//	for (int k=0; k<iT; k++) BondArrayCollision[k].ResetBond();
	...
	for (int i=0; i<NumVox(); i++) 
	{
		...
		{
            // VoxArray[i].growthTime = pEnv->pObj->GetGrowthTime(i) * (GetStopConditionValue() - GetInitCmTime());
		...
		else
		{
		    if(pEnv->pObj->GetUsingFinalVoxelSize())
                ...
                VoxArray[i].growthTime  = 0.05; // GetStopConditionValue() - VoxArray[i].startGrowthTime;
            else
...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
	...
	if(CurTime >= 0.75*StopConditionValue && SS.RegimeStartCM == Vec3D<>(0,0,0)) // Only relevant for Evo: last 25% of exec time considered as regime
	{
...
bool CVX_Sim::StopConditionMet(void) //have we met the stop condition yet?
{
	...
	switch(StopConditionType){
		...
		case SC_MAX_SIM_TIME:
			if(pEnv->pObj->GetUsingFinalVoxelSize())
		 		return CurTime>(1.25*StopConditionValue) ? true:false; // If EvoDevo keep going for another 0.25*StopConditionValue to later allow regime comparison with Evo
		 	else
...
bool CVX_Sim::UpdateStats(std::string* pRetMessage) //updates simulation state (SS)
{
	...
	if (CCom) SS.CurCM = GetCM(); //calculate center of GetMass
