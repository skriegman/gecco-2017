	...
	switch (CurViewCol) {
		case RVC_TYPE:
            ...
            if (pSim->pEnv->pObj->GetUsingInitialVoxelSize() || pSim->pEnv->pObj->GetUsingFinalVoxelSize())  // color difference from nominal
            {
				...
				double normSize = (finalSize - minSize ) / (maxSize - minSize);

				if(finalSize < pSim->VoxArray[SIndex].GetNominalSize())  // we care about just the change
					return CColor(0.0f, 0.0f, normSize+0.2f, 1.0f); // shrinking
				else
					return CColor(normSize+0.2f, 0.0f, 0.0f, 1.0f); // expanding
