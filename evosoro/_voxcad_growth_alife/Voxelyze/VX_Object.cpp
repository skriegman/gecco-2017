...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	// FC: Let's look for GrowthRate tag in the xml file (vxa)
	if (pXML->FindElement("GrowthRate"))
	{ 
		usingGrowthRate = true;
		int voxCounter = 0;

		InitGrowthRateArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetGrowthRate(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
