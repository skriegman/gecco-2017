...
// 	// std::cout << "totalPoints: " << totalPoints << std::endl;
	...
	// FC Here we print the fitness values we're interested in for growth
	if(pEnv->getSourcesPresent())
	{
		...
		{					
			pXML->Element("MinDistanceFromSource_" + environmentalSources[kk]->getSourceName(), environmentalSources[kk]->getMinimumDistanceToRobot());
			pXML->Element("AverageMinDistanceFromSource_" + environmentalSources[kk]->getSourceName(), environmentalSources[kk]->getAverageMinimumDistanceToRobot());
		}
	}


	// OUTPUT THE NUMBER OF VOXEL
	pXML->Element("VoxelNumber", NumVox()); // TODO Check: does this include other voxels possibly present in the environment, that do not belong to the robot?

	// TODO_FC print here chull volumes and/or directly the branching index and symmetry index
