...
VOXELYZE_VERSION = '_voxcad_forTransfer'
...
RUN_DIR = "transfer_data"  # Subdirectory where results are going to be generated
RUN_NAME = "Transfer"
...
class MyGenotype(Genotype):
    def __init__(self):
        ...
        self.to_phenotype_mapping.add_map(name="material", tag="<Data>", func=make_material_tree,
                                          dependency_order=["shape", "muscleOrTissue", "muscleType"],
                                          output_type=int)
...
my_env = Env(sticky_floor=0, time_between_traces=0)  # , frequency=0.5)  # , actuation_variance=0.357)
# actuation_variance = 0.357 = 5^(1/3) / 2^(1/3) - 1
