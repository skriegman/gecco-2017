...
class MyGenotype(Genotype):
    def __init__(self):
        ...
        self.add_network(DirectEncoding(output_node_name="initial_size", orig_size_xyz=IND_SIZE, scale=1, p=1/48.))
        self.to_phenotype_mapping.add_map(name="initial_size", tag="<InitialVoxelSize>")

        self.add_network(DirectEncoding(output_node_name="final_size", orig_size_xyz=IND_SIZE, scale=1, p=1/48.))
        self.to_phenotype_mapping.add_map(name="final_size", tag="<FinalVoxelSize>")

        self.add_network(DirectEncoding(output_node_name="phase_offset", orig_size_xyz=IND_SIZE, scale=1, p=1/48.))
        self.to_phenotype_mapping.add_map(name="phase_offset", tag="<PhaseOffset>")
        #
        self.add_network(DirectEncoding(output_node_name="final_phase_offset", orig_size_xyz=IND_SIZE, scale=1, p=1/48.))
